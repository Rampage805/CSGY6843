...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        icmp_type, code, myChecksum, packetID, sequence = struct.unpack('bbHHh', icmp_header)
        if packetID == ID:
            size = struct.calcsize('d')
            timeSent = struct.unpack("d", recPacket[28: 28 + size])[0]
            packet_header = recPacket[0:20]
            packet_ttl = packet_header[8:9]
            data = recPacket[28:]
            delay = (timeReceived - timeSent) * 1000
            ttl = struct.unpack('B', packet_ttl)
...
def ping(host, timeout=1):
    ...
    print("4 packets are transmitted, 4 packets received: {}, packet loss: {}%".format(len(delays), round((4.0 - len(delays)) / 4.0 * 100, 1)))
    if len(delays) > 0:
