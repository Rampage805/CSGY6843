        ...
        for tries in range(TRIES):
            ...
            try:
                ...
                if type == 11:
                    ...
                    print("%.0fms\t%s\t%s" % ((timeReceived - t) * 1000, addr[0], routerHostname))
                    #Fill in end
                elif type == 3:
                    bytes = struct.calcsize("d")
                    timeSent = struct.unpack("d", recvPacket[28:28 + bytes])[0]
                    ...
                    print("%.0fms\tI%s\t%s" % ((timeReceived - t) * 1000, addr[0], routerHostname))
                    #Fill in end
                elif type == 0:
                    bytes = struct.calcsize("d")
                    timeSent = struct.unpack("d", recvPacket[28:28 + bytes])[0]
                    ...
                    print("%.0fms\t%s\t%s" % ((timeReceived - timeSent) * 1000, addr[0], routerHostname))
                    return
                    #Fill in end
                break
            finally:
